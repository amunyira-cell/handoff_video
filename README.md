...
<title>Recovery Navigator App — Video Flow</title>
<style>
  ...
  .btn.outlined-blue{background:transparent;border:2px solid var(--blue);color:var(--blue)}
...
<div id="rolePage" class="page active">
  ...
  <div class="card">
    ...
    <button class="btn blue" onclick="goTo('loginPage','patient')">👤 Patient Login</button>
    <button class="btn green" style="margin-top:8px" onclick="goTo('loginPage','navigator')">🧭 Navigator Login</button>
    <p class="small">Single-file demo — no real authentication. Enter any email/password to continue.</p>
  </div>
</div>
...
<div id="registerPage" class="page">
  <div class="card">
    ...
    <div class="actions">
      ...
      <button class="btn outlined-blue half" onclick="back()">Back</button>
    </div>
  </div>
</div>
...
<div id="loginPage" class="page">
  <div class="card">
    ...
    <p class="small" style="margin-top:10px;text-align:center">Don’t have an account? <a href="#" onclick="goToRegister()" style="color:#2563eb;text-decoration:underline">Create one</a></p>
  </div>
</div>
...
<div id="patientPage" class="page">
  <div class="card">
    ...
    <div class="list-card">
      <div style="font-weight:700;color:#dc2626">📞 Emergency Help</div>
      <a href="tel:911" class="patient-row"><span>🚑 Call Emergency (911)</span></a>
      <a href="tel:988" class="patient-row"><span>💬 Suicide & Crisis Lifeline (988)</span></a>
      <a href="tel:+18001234567" class="patient-row"><span>🏥 Local Recovery Center</span></a>
      <a href="tel:+18007654321" class="patient-row"><span>☎️ 24/7 Helpline</span></a>
    </div>

    <div class="list-card">
      <div style="font-weight:700;color:var(--blue)">📅 Follow-Up Dashboard</div>
      <div class="patient-row"><div>📞 Recent Calls</div><button class="btn-ghost" onclick="pageStack.push('patientPage'); loadRecentCalls(); showPage('recentCallsPage')">Open</button></div>
      <div class="patient-row"><div>📔 Appointments</div><button class="btn-ghost" onclick="pageStack.push('patientPage'); loadAppointments(); showPage('appointmentsPage')">Open</button></div>
      <div class="patient-row"><div>🧭 Recovery Plan</div><button class="btn-ghost" onclick="pageStack.push('patientPage'); loadRecovery(); showPage('recoveryPage')">Open</button></div>
      <div>
        <div style="font-weight:700;margin-bottom:6px">💬 Symptoms & Comments</div>
        ...
        <div class="actions" style="margin-top:8px">
          ...
          <button class="btn outlined-blue" onclick="back()">Back</button>
        </div>
    ...
    <div class="list-card">
      <div class="patient-row">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;width:48%">
          ...
          <button class="btn green" onclick="startConnecting('John Doe')">🎥 Start Video Call</button>
        </div>
      </div>
      <div class="patient-row" style="margin-top:12px">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;width:48%">
          ...
          <button class="btn green" onclick="startConnecting('Sarah Lee')">🎥 Start Video Call</button>
        </div>
      </div>
      <div class="patient-row" style="margin-top:12px">
        <div>
        ...
        <div style="display:flex;flex-direction:column;gap:8px;width:48%">
          ...
          <button class="btn green" onclick="startConnecting('Mark Johnson')">🎥 Start Video Call</button>
        </div>
      </div>
    </div>

    <div class="patient-row"><div>📞 Recent Calls</div><button class="btn-ghost" onclick="pageStack.push('navigatorPage'); loadRecentCalls(); showPage('recentCallsPage')">Open</button></div>
...
<div id="connectingPage" class="page">
  <div class="card center">
    <h1>🔗 Warm Handoff — Connecting</h1>
    <div class="status" id="connectingStatus">Calling…</div>
...
<div id="videoPage" class="page">
  <div class="card">
    ...
    <div class="controls">
      ...
      <button class="btn outlined-blue" style="width:160px" onclick="back()">Back</button>
    </div>
  </div>
</div>
...
<!-- Recent Calls Page -->
<div id="recentCallsPage" class="page">
  <div class="card">
    <h1>📞 Recent Calls</h1>
    <p>View your most recent video calls.</p>
    <div id="recentCallsList" style="background:#f9fafb;border-radius:8px;padding:10px;margin-bottom:12px;font-size:14px">
      <i>No recent calls yet.</i>
    </div>
    <div class="actions">
      <button class="btn outlined-blue half" onclick="back()">Back</button>
    </div>
...
<div id="toast" style="
  ...
  pointer-events: none;
  z-index: 9999;
">Entry posted successfully.</div>

<footer>Demo — single HTML file. Use Back to navigate between pages.</footer>
...
function openPatient(name){
  ...
  pageStack.push(current);
  showPage('recordPage');
  document.getElementById('recordHeading').innerText = name + ' — Record';
  ...
  symptomContainer.innerHTML = log.length ? log.map(e => `<div>🕓 ${e}</div>`).join('') : '<i>No symptoms logged yet.</i>';
  ...
  commentContainer.innerHTML = comments.length ? comments.map(c => `<div>💬 ${c}</div>`).join('') : '<i>No navigator comments yet.</i>';
...
function addNavComment(){
  ...
  const entry = new Date().toLocaleString() + ' — ' + txt;
...
// Symptoms
  function postSymptom(){
    ...
    entry.textContent = new Date().toLocaleString() + ' — ' + txt;
  ...
  function endCall(){
    // Log recent call
    if(pendingPatientName){ logRecentCall(pendingPatientName); }
...
function loadAppointments(){
  ...
  container.innerHTML = list.length ? list.map(a => `<div>📌 ${a}</div>`).join('') : '<i>No appointments yet.</i>';
}
function addAppointment(){
  const txt = document.getElementById('newAppointment').value.trim();
  const list = JSON.parse(localStorage.getItem('appointments') || '[]');
  if(txt){ list.unshift(new Date().toLocaleString() + ' — ' + txt); }
...
function loadRecovery(){
  ...
  container.innerHTML = list.length ? list.map(s => `<div>✅ ${s}</div>`).join('') : '<i>No recovery steps yet.</i>';
}
...
function loadRecentCalls(){
  const list = JSON.parse(localStorage.getItem('recentCalls') || '[]');
  const container = document.getElementById('recentCallsList');
  container.innerHTML = list.length ? list.map(c => `<div>📞 ${c}</div>`).join('') : '<i>No recent calls yet.</i>';
}

function logRecentCall(name){
  const list = JSON.parse(localStorage.getItem('recentCalls') || '[]');
  const entry = new Date().toLocaleString() + ' — ' + name;
  list.unshift(entry);
  localStorage.setItem('recentCalls', JSON.stringify(list));
}
